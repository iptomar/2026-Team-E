...
function DraggableComponent({ config }: { config: ComponentConfig }) {
    ...
    return (
        <div
            ...
            {...listeners}
            {...attributes}
            className={`flex cursor-grab items-center gap-3 rounded-xl border border-gray-200 bg-white p-4 transition-all duration-200 hover:border-indigo-300 hover:shadow-lg hover:shadow-indigo-100 ${isDragging ? 'scale-105 opacity-70 shadow-xl ring-2 ring-indigo-400' : ''}`}
        >
